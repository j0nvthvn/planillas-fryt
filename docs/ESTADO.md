...
- **Encabezado al estilo Mercado Pago** (commit `c180116`, 2026-09-17, en producción):
  ...
  - Los encabezados con "volver" quedan fijos en el celular.
  - **Barra de estado** (`lib/barraEstado.ts`, `styles.css`, 2026-09-17): la banda de Hoy y de Ajustes llega bajo la hora y la batería. La franja del `safe-area` la pinta `.franja-barra`, un bloque fijo del Layout que repite el degradado de la banda con el mismo `background-size` (`--banda-h` + `--safe-top`), así que no hay costura; la banda sube hasta el borde y el `main` la recorta. Mientras la banda cubre el tope, un `IntersectionObserver` marca `data-banda` en `<html>` y el `theme-color` va índigo (íconos blancos); al bajar, franja y `theme-color` vuelven a `--canvas` a la vez, de modo que el sistema siempre elige íconos que se distinguen. Antes el `theme-color` iba índigo pero la franja la pintaba el body en gris claro: íconos blancos invisibles en Hoy y Ajustes. `--safe-top` (en vez de `env()` suelto) permite simular la barra en el navegador y en el e2e. iOS instalada ignora el `theme-color`.
...
- **Limpieza interna de la app** (2026-09-19, mismo pase): el patrón de la casa —lógica pura en un `.ts` al lado de la pantalla, con test— se aplicó donde faltaba.
  ...
  - Duplicación: `lib/metodos.ts` (mata cuatro `as unknown as`), `features/analisis/api.ts` (`resumen_periodo` estaba copiada en dos pantallas), `useQuitarMarcaDia` (tres copias), `Dato`/`EstadoChip` a `components/`, un solo `ordenTurno`, y las query keys sueltas de vuelta en `qk` (`qk.metodos`/`qk.trabajadores` pasan a ser funciones: la key se armaba por spread y la invalidación funcionaba por accidente de prefijo).
