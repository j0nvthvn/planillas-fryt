...
export function DeltaBadge({ actual, anterior }: { actual: number; anterior: number | null | undefined }) {
  ...
  if (anterior === 0) {
    ...
    return <span className="text-[11px] font-semibold text-muted2">nuevo</span>
  }
  const diff = actual - anterior
  const sube = diff > 0
  const cls = `inline-flex items-center gap-0.5 text-[11px] font-bold ${sube ? 'text-pos' : 'text-neg'}`
  ...
  if (Math.abs(pct) < 0.5) return <span className="text-[11px] font-medium text-muted2">≈ igual</span>
