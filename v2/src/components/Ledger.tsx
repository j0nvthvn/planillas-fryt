...
export function LedgerHead({ label, right = '$' }: { label: string; right?: ReactNode }) {
  return (
    <div className="flex items-baseline justify-between gap-3 pt-3.5 pb-1.5 border-b border-brand text-[11px] font-bold uppercase tracking-[0.12em] text-brand">
...
export function LedgerLine({ label, hint, value, color, muted, onClick, dot }: {
  ...
  return (
    <Tag type={onClick ? 'button' : undefined} onClick={onClick}
      className={`w-full flex items-baseline justify-between gap-3 py-2 min-h-[44px] border-b border-hairline text-[14.5px] text-left ${onClick ? 'hover:bg-soft/60 active:bg-soft' : ''}`}>
      <span className="min-w-0 flex items-baseline gap-2 text-ink">
        ...
        {hint && <span className="text-[11.5px] text-muted shrink-0">{hint}</span>}
      </span>
...
export function LedgerTotal({ label, value, color, size = 'lg' }: { label: string; value: number; color?: string; size?: 'lg' | 'sm' }) {
  return (
    <div className={`flex items-baseline justify-between gap-3 ${size === 'lg' ? 'pt-2.5 mt-0.5 border-t border-dashed border-brand' : 'pt-1'}`}>
      <span className={size === 'lg' ? 'font-bold text-ink' : 'text-[13px] text-muted'}>{label}</span>
      <span className={`${size === 'lg' ? 'amount text-[24px]' : 'tabular-nums font-semibold text-[14px]'}`} style={color ? { color } : undefined}>{clp(value)}</span>
    </div>
  )
}
