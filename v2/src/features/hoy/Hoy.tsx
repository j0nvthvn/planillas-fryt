    ...
    <div className="max-w-2xl mx-auto">
      <header className="flex items-center justify-between gap-3 mb-[18px]">
        <div className="min-w-0">
          ...
          <h1 className="font-display text-[20px] font-semibold leading-[1.1] tracking-[-0.02em] text-ink">Hoy</h1>
        </div>
        <AvatarMenu />
      </header>
      ...
      {resumen.isPending && turnos.isPending ? <EsqueletoContenido sinTitulo /> : (
        <>
          ...
          <section aria-label="Neto del día" className="card pb-4 mb-3">
            ...
            <p className="amount text-hero leading-[1.05] tracking-[-0.03em] text-ink mt-2.5">{clp(neto)}</p>
            {hayAyer && (
          ...
          {cta ? (
            <button type="button" onClick={() => void navigate({ to: '/turno', search: { fecha, modo: cta.modo } })} className="btn-primary w-full min-h-[52px] text-[15px] rounded-[14px] mb-3">
            ...
            <div className="mb-3 rounded-[14px] bg-pos-tint border border-pos-border px-4 py-3 min-h-[52px] flex items-center justify-between gap-3">
              <p className="text-[15px] font-semibold text-pos">Día completo registrado</p>
          ...
          <section className="card p-0 overflow-hidden mb-3" aria-label="Efectivo esperado en caja">
            <div className="px-[18px] pt-4 pb-3.5">
              <div className="flex items-baseline justify-between gap-2.5">
                <p className="text-sm font-medium text-ink2">Efectivo esperado</p>
                <span className="cifra text-[20px] tracking-[-0.02em] text-ink">{clp(efectivoEsperado)}</span>
              </div>
            ...
            {metodosVisibles.map((m) => (
              <div key={m.key} className="flex items-center gap-3 px-[18px] py-2.5 min-h-[60px] border-t border-hairline">
                <MetodoLogo metodo={m} />
                <span className="flex-1 min-w-0 text-sm font-medium text-ink truncate">{m.label}</span>
                <span className="text-xs text-muted2 tabular-nums">{totalVentas ? Math.round((m.monto / totalVentas) * 100) : 0}%</span>
                <span className="cifra text-[15px] text-ink min-w-[86px] text-right">{clp(m.monto)}</span>
              </div>
            ))}
            {porMetodo.length > 3 && (
              <button type="button" onClick={() => setVerMetodos((v) => !v)} aria-expanded={verMetodos}
                className="w-full flex items-center justify-center gap-1.5 min-h-[52px] border-t border-hairline text-[13px] font-semibold text-brand hover:bg-soft">
            ...
            <div className="grid grid-cols-2 gap-2.5">
              {lista.map(({ turno }) => (
                <Link key={turno.id} to="/dia" search={{ fecha }} className={`rounded-[14px] bg-card border border-hairline shadow-card p-3.5 flex flex-col hover:border-hairline-strong transition-colors ${turno.modo === 'completo' ? 'col-span-2' : ''}`}>
                  <div className="flex items-center justify-between gap-1.5 mb-2">
                    <span className="text-[13px] font-medium text-ink2">{etiquetaModo(turno.modo)}</span>
                  ...
                  <span className="cifra text-[20px] leading-none tracking-[-0.02em] text-ink">{clp(turno.total_ventas)}</span>
              ...
              {pendiente && (
                <div className="rounded-[14px] bg-card border border-dashed border-hairline-strong p-3.5 flex flex-col">
                  <div className="flex items-center justify-between gap-1.5 mb-2">
                    <span className="text-[13px] font-medium text-ink2">{etiquetaModo(pendiente)}</span>
                    <span className="badge bg-soft text-muted">Pendiente</span>
                  </div>
                  <span className="cifra text-[20px] leading-none text-muted2">—</span>
...
function Dato({ label, className = 'text-ink', children }: { label: string; className?: string; children: ReactNode }) {
  return (
    <div className="flex-1 min-w-0">
      ...
      <p className={`cifra text-[15px] leading-none truncate ${className}`}>{children}</p>
    </div>
  )
}
