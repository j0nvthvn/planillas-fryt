            ...
            <div className="h-[220px]" aria-hidden="true">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={dias} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                  ...
                  <XAxis dataKey="label" tick={{ fontSize: 12, fill: 'var(--muted)' }} interval="preserveStartEnd" />
                  <YAxis tickFormatter={clpCorto} tick={{ fontSize: 12, fill: 'var(--muted)' }} width={48} />
            ...
            <div className="card">
              ...
              {r.top_proveedores.length === 0 ? <p className="text-sm text-muted">Sin compras en el período.</p> : r.top_proveedores.map((p) => (
                <Link key={p.proveedor_id ?? p.nombre} to={p.proveedor_id ? '/proveedores/$id' : '/proveedores'} params={{ id: p.proveedor_id ?? '' }} className="flex items-center gap-3 py-2 border-b border-soft last:border-0">
                  ...
                  <span className="flex-1 min-w-0"><span className="block text-sm font-medium text-ink truncate">{p.nombre}</span><span className="block text-xs text-muted">{p.compras} compra{p.compras === 1 ? '' : 's'}</span></span>
                  <span className="font-semibold tabular-nums text-ink text-sm">{clp(p.monto)}</span>
                </Link>
              ))}
            </div>
          </div>
...
function TooltipDia({ active, payload }: { active?: boolean; payload?: { payload?: Record<string, unknown> }[] }) {
  ...
  return (
    <div className="rounded-xl bg-card border border-hairline shadow-hero px-3 py-2 text-xs min-w-[170px]">
