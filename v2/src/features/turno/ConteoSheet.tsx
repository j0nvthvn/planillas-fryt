...
export function ConteoSheet({ inicial, esperado, onAccept, onTotalManual, onClose }: Props) {
  ...
  return (
    <BottomSheet title="Conteo de caja" onClose={onClose} footer={<>
      <button type="button" className="btn-primary w-full min-h-[50px] text-[15px]" disabled={vacio} onClick={() => onAccept(total, conteo)}>
        <Icon name="check" className="w-[18px] h-[18px]" stroke={2.4} />Registrar conteo
      </button>
      <div className="flex justify-between gap-2">
        <button type="button" className="btn-ghost min-h-[40px] px-2.5 text-[13px] text-ink2" onClick={onTotalManual}>Escribir el total a mano</button>
        {!vacio && <button type="button" className="btn-ghost min-h-[40px] px-2.5 text-[13px] text-neg" onClick={() => setConteo({})}>Empezar de nuevo</button>}
      </div>
    </>}>
      <div className="rounded-[14px] bg-soft border border-hairline px-4 py-3.5">
        <p className="eyebrow mb-2">Contado</p>
        <p className="amount text-amount leading-none tracking-[-0.03em] text-ink" aria-live="polite">{clp(total)}</p>
        <p className="text-[13px] text-muted mt-[9px] tabular-nums">
        ...
        {DENOMINACIONES.map((d) => {
          ...
          return (
            <li key={d} className="flex items-center gap-2">
              <span className="w-[66px] shrink-0 cifra text-sm text-ink">{clp(d)}</span>
              <button type="button" aria-label={`Quitar un ${clp(d)}`} disabled={n === 0}
                onClick={() => setConteo((c) => ajustar(c, d, -1))}
                className="w-[42px] h-[42px] shrink-0 rounded-[11px] bg-soft border border-hairline text-ink2 grid place-items-center disabled:opacity-50">
              ...
              <input
                ...
                className="w-[54px] h-[42px] shrink-0 rounded-[11px] bg-card border border-hairline-strong text-center px-1 font-semibold tabular-nums text-ink placeholder:text-muted2 focus:outline-none focus:border-brand focus:ring-1 focus:ring-brand"
              />
              <button type="button" aria-label={`Agregar un ${clp(d)}`}
                onClick={() => setConteo((c) => ajustar(c, d, 1))}
                className="w-[42px] h-[42px] shrink-0 rounded-[11px] bg-soft border border-hairline text-ink2 grid place-items-center">
                <Icon name="plus" className="w-4 h-4" stroke={2.4} />
              </button>
              <span className={`flex-1 text-right text-[13px] font-semibold tabular-nums ${n ? 'text-ink2' : 'text-muted2'}`}>{clp(d * n)}</span>
            </li>
          )
        })}
      </ul>
