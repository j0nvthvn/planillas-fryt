...
function porSemana(dias: Dia[]): { desde: string; hasta: string; ventas: number; prov: number; neto: number; conRegistro: number; alerta: boolean }[] {
  const grupos = new Map<string, ReturnType<typeof porSemana>[number]>()
  for (const d of dias) {
    const lunes = sumarDias(d.fecha, -((diaSemana(d.fecha) + 6) % 7))
    const g = grupos.get(lunes) ?? { desde: d.fecha, hasta: d.fecha, ventas: 0, prov: 0, neto: 0, conRegistro: 0, alerta: false }
    g.hasta = d.fecha
...
export function completarDias(desde: string, hasta: string, dias: Dia[]): Dia[] {
  ...
  for (let f = desde; f <= hasta; f = sumarDias(f, 1)) {
    todos.push(porFecha.get(f) ?? { fecha: f, turnos: 0, total_ventas: 0, total_proveedores: 0, neto: 0, estado: 'sin_registro', corregido: false, con_descuadre: false, tiene_borrador: false })
  }
  return todos
}
...
export function correoPeriodo(entrada: DatosPeriodo): Correo {
  ...
  const detalleHeroe = [
    ...
    d.tipo !== 'diario' ? `<br><span style="display:inline-block;margin-top:8px;font-size:13px;color:${T.muted};">${conRegistro.length} de ${totalDias} días con registro</span>` : '',
  ].join('')
...
${boton(d.tipo === 'diario' ? 'Ver la planilla del día' : 'Ver el análisis', enlace)}`)
  ...
  const alertas = pendientes.length
  ...
  if (d.tipo === 'semanal') {
    detalle = seccion('Día a día', lista(d.dias.map((x, i) => fila({
      ...
      sub: x.turnos ? `Ventas ${clp(x.total_ventas)} · Prov. ${clp(x.total_proveedores)}` : 'Sin registro',
    ...
    detalle = seccion('Semana a semana', lista(semanas.map((s, i) => fila({
      ...
      sub: `Ventas ${clp(s.ventas)} · Prov. ${clp(s.prov)} · ${s.conRegistro} día${s.conRegistro === 1 ? '' : 's'}`,
      monto: clp(s.neto),
